...
import cv2
import Weapon_Dict
from Damage_OCR import getDamage, Damage_Correction


def APEX_Chart_Analyze(Video_Path, FRAMES, WEAPONS, AMMOS, DAMAGES, total_frames, fps=60, EvnChartExcel_Path=None, FiringList_Path=None, rank_league = True, Firing_List_FwdFrame = 9, Save_to_BigData = False):
    ...
    #开火报表
    Firing_List = list()#[VideoName, firing_start_f, firing_end_f, used_weapon, used_ammo, damage_dealt]
    firing_start_f = 0
    ...
    def Round_Report():#一梭子汇报
        ...
        if not weapon_before: return None#之前没武器
        ...
        if capture_p == frame_num:
            ...
            ret, Img = capture_frame.read()
        capture_p += 1
        damage = getDamage(Img, rank_league)
        damage_fixed = Damage_Correction(damage_fixed, damage)
        damage_ThisRound = damage_fixed - damage_before
        damage_before = damage_fixed
        
        print('frame:{}, 使用{}打掉{}发弹药, DMG总:{}, DMG本次:{}'.format(frame_num, used_weapon, shot_ammo, damage_fixed, damage_ThisRound))
        print('frame:{}, 使用{}打掉{}发弹药, DMG总:{}, DMG本次:{}'.format(frame_num, used_weapon, shot_ammo, damage_fixed, damage_ThisRound), file=txtdata)
        
        Firing_List.append([Video_Name, firing_start_f, frame_num, used_weapon, shot_ammo, damage_ThisRound])
        shooting = False
    ...
    for frame_num in range(total_frames):
            ...
            if not ammo is None:
                ...
                if weapon_change_done:#换了
                    ...
                    if ammo < ammo_after:#子弹少了
                        bps = Weapon_Dict.Dict_Weapon[weapon].Bps#三重3，滋蹦2，其余1
        ...
        #End Loop
        if weapon_change_done: weapon_change_done = False
    txtdata.close()
    ...
    xd.to_excel(EvnChartExcel_Path, header=['Frame', 'Ammo', 'AmmoBefore', 'AmmoAfter', 'Shooting', 'shot_pauseflag', 'shot_pause_delayframes', 'Weapon', 'WeaponHold', 'WeaponBefore', 'weapon_temp', 'weapon_change', 'weapon_changedone', 'weapon_change_delayframes', 'Damage', 'damage_fixed', 'damage_before', 'damage_ThisRound'], index=None)
    
    xdfl = pd.DataFrame(Firing_List, columns=['Video_Name', 'Start_Frame', 'End_Frame', 'Used_Weapon', 'Used_Ammo', 'Damage_Dealt'])
...
if __name__ == '__main__':
    ...
    APEX_Chart_Analyze(vid_path, FRAMES, WEAPONS, AMMOS, DAMAGES, total_frames, fps, Evncht_path, Firlst_path, True)
