...
def ms2timestr(ms):
    ...
    time = str(time_min) + ':' + str(time_s).zfill(2)
    return time
...
def Round_Report():#一梭子汇报
    ...
    print('frame:{}, time:{}, 使用{}打掉{}发弹药, DMG总:{}, DMG本次:{}'.format(i, ms2timestr(time_ms), used_weapon, shot_ammo, damage_fixed, damage_ThisRound))
    print('frame:{}, time:{}, 使用{}打掉{}发弹药, DMG总:{}, DMG本次:{}'.format(i, ms2timestr(time_ms), used_weapon, shot_ammo, damage_fixed, damage_ThisRound), file=txtdata)
...
def Temp_Frame_Save(Img, time, weapon, ammo):
    global tempi
    print('case:{}, frame:{}, time:{}, weapon:{}, ammo:{}'.format(tempi, i, time, weapon, ammo))
    print('case:{}, frame:{}, time:{}, weapon:{}, ammo:{}'.format(tempi, i, time, weapon, ammo), file=txtdata)
...
if __name__ == '__main__':
    ...
    #开火报表
    Firing_List = list()#[firing_start_f, firing_end_f, used_weapon, used_ammo, damage_dealt]
    firing_start_f = 0
    ...
    xd.to_excel('./Temp/EventChart.xlsx', header=['Frame', 'Ammo', 'AmmoBefore', 'AmmoAfter', 'Shooting', 'shot_pauseflag', 'shot_pause_delayframes', 'Weapon', 'WeaponHold', 'WeaponBefore', 'weapon_temp', 'weapon_change', 'weapon_changedone', 'weapon_change_delayframes', 'Damage', 'damage_fixed', 'damage_before', 'damage_ThisRound'], index=None)
    
    xdfl = pd.DataFrame(Firing_List)
    xdfl.to_excel('./Temp/Firing_List.xlsx', header=['Start_Frame', 'End_Frame', 'Used_Weapon', 'Used_Ammo', 'Damage_Dealt'], index=None)