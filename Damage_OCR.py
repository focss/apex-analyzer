...
def DamageArea_Select(Img, Rank = False):
    if Rank:
        return Img[94:120,1675:1761]
    return Img[94:120,1749:1835]#裁剪

def CutDmgLogo_Classic(Img_Cut):
...
def CutDmgLogo_MatchTpl(Img_Cut):
    Img_Cut = cv2.threshold(Img_Cut, 190, 255, cv2.THRESH_BINARY_INV)[1]#二值化
    Img_Cut = cv2.cvtColor(Img_Cut, cv2.COLOR_BGR2GRAY)
...
def getDamage(img, Rank=False)->int:
    Img_Cut = DamageArea_Select(img, Rank=Rank)
    Img_DmgNum = CutDmgLogo_MatchTpl(Img_Cut)
    if Img_DmgNum.shape[1] == 0: return 0
    ...
    if len(Text_Damage): return int(Text_Damage)
    return 0
...
if __name__ == '__main__':
    ...
    Img_DmgNum = CutDmgLogo_Classic(Img_Cut)
